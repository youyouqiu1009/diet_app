...
- **JS/CSS を変更して push する前に、`index.html` 内のアセット読み込みの `?v=` の数字をすべて +1 する**（`style.css?v=N`、`app.js?v=N`、`calorie.js?v=N`、`weight.js?v=N` の4箇所を同じ番号に揃える）。
  理由: ブラウザ（特にiOS Safari）が古いJS/CSSをキャッシュしたまま動かす問題が繰り返し発生したため。
- Service Worker は意図的に導入していない（オフライン要件がなく、キャッシュ問題を悪化させるため）。
